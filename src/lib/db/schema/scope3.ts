...
export const supplierEmissions = pgTable(
  "supplier_emissions",
  {
    id: uuid("id").defaultRandom().primaryKey(),
  ...
  (table) => ({
    companyIdx: index("idx_supplier_emissions_company_id").on(table.companyId),
  })
);

export const scope3Records = pgTable(
  "scope3_records",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    companyId: uuid("company_id")
  ...
  (table) => ({
    companyIdx: index("idx_scope3_records_company_id").on(table.companyId),
  })
);
